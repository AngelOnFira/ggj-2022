...
func _process(delta):
	...
	if self.current_index_in_text_to_add >= len(self.text_to_add[0]["text"]):
		...
		self.current_text = "[" + text_to_add[0]["format"] + "]" + cumulate_string(text_to_add[0]["text"],0,len(text_to_add[0]["text"])-1) + "[/" + text_to_add[0]["format"] + "]"
	...
	if update_flag and len(self.text_to_add) > 0:
		...
		self.current_text =  "[" + text_to_add[0]["format"] + "]" + cumulate_string(text_to_add[0]["text"],0,self.current_index_in_text_to_add) + "[/" + text_to_add[0]["format"] + "]" +"\n\n"
